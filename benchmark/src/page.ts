...
// driver. Both engines get a fixed 120x40 grid, the same font, and
// byte-identical workloads. xterm runs with its WebGL addon (its fastest
// renderer); rioterm runs its canvas renderer.
...
declare global {
  interface Window {
    bench: (engine: 'xterm' | 'rioterm', scenario: string) => Promise<Record<string, number>>;
    benchInit: (engine: 'xterm' | 'rioterm') => Promise<number>;
  }
}
...
window.benchInit = async (which) => {
  ...
  host().textContent = '';
  const start = performance.now();
  current = which === 'xterm' ? await makeXterm(host()) : await makeRioterm(host());
